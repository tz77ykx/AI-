...
大多数人把 Claude 当成搜索框来用。问完问题，得到答案，关掉标签页。他们把最强大的功能束之高阁。

建议收藏 :)

斯坦福开发了一套名为 STORM 的研究系统。在同行评审测试中，它产出的文章组织性比次优方法高出 **25%**。它是开源的。它是免费的。几乎没人知道，你可以用 4 个提示词在 Claude 中复现同样的思路。

无需软件。无需 GitHub。无需配置。直接粘贴。5 分钟后，你对某个主题的了解将超过那些花几天阅读的人。
